...
# Supplier مضافة بناءً على تحليل الارتباط
...
class Product(models.Model):
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, null=True)  # ✅ جديد
...
class InventoryMovement(models.Model):
    ...
    def __str__(self):
        return f"{self.get_movement_type_display()} | {self.quantity} | {self.material_type}"
...
class MaterialRequest(models.Model):
    ...
    def __str__(self):
        return f"{self.status.capitalize()} Request: {self.raw_material.name} ({self.quantity} {self.raw_material.unit})"
